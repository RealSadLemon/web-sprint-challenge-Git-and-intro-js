...
function listOfNames(arr) {
  const newArr = [...arr];
  const finalArr = [];
  for(let eachArtist of newArr){
    finalArr.push(eachArtist.name);
  }
  return finalArr;
}
...
function addArtist(arr, name, years, genre, nationality, bio) {
  const obj = {name: name, years: years, genre: genre, nationality: nationality, bio: bio};
  arr.push(obj);
  return arr;
}
...
function lotsOfArt(arr) {
  ...
  for(let i = 0; i < newArr.length; i++){
    if(newArr[i].paintings > 100){
      finalArr.push(newArr[i].name);
    }
  }
  return finalArr;
}
